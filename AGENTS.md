...
| `app/warble/play/` | Warble itself, playable. Linked from its study as `live` |
...
That rule is why `components/warble/icons.tsx` exists. Warble arrived importing twelve
`lucide-react` icons, each used once; they are inline SVGs now, with lucide's geometry and
defaults so a thirteenth can be lifted straight from source.
