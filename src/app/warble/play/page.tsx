...
/*
 * Two React Compiler rules are off for this file only. Each flags a real
 * pattern; neither is a bug here, and both would mean restructuring a working
 * audio scheduler that can only really be verified by ear.
 *
 * - **`react-hooks/immutability`** — two functions are referenced above their
 *   own declaration: `scheduler` schedules its next tick with a `setTimeout` to
 *   itself, and the mount effect calls `loadSlot`, which is defined below it.
 *   Both resolve at call time, by which point the binding exists. The linter
 *   cannot prove that; the runtime never hits it.
 * - **`react-hooks/set-state-in-effect`** — the mount effect reads dark mode,
 *   the URL hash and the saved slot out of `localStorage` and sets state from
 *   them, and the transport effect resets the step marker on stop. Both cost
 *   one extra render on an event that happens once. Restoring persisted state
 *   after mount is the intended shape of this, not an accident.
 *
 * Scoped to this file deliberately — these rules stay on everywhere else.
 */
...
import {
  ...
  Sun,
  Volume2,
} from '@/components/warble/icons';
import Link from 'next/link';

/**
 * Warble — a procedural ringtone generator on the Web Audio API.
 *
 * Ported from the old site on 2026-08-15, essentially unchanged. Two things
 * were altered and both were about this codebase rather than about Warble:
 *
 * - **`lucide-react` is gone.** Twelve icons, each used once, are inline in
 *   `components/warble/icons.tsx`. Dependencies here are `next`, `react` and
 *   `react-dom`, deliberately.
 * - **The page-transition wrapper is gone**, and the `Back` link's old
 *   `#experiments` target went with it — that section does not exist here. It
 *   points at `/warble`, the case study, which is where a visitor came from.
 *
 * **It lives at `/warble/play`, not `/warble`.** The study owns the canonical
 * path; a static `app/warble/page.tsx` would have shadowed it, because a static
 * segment beats the `[slug]` route. With only `play` under `app/warble/`, the
 * parent path falls through to `[slug]` as intended — verified, not assumed.
 *
 * It keeps its own light/dark toggle and its own visual language, which do not
 * match the rest of the site. That is on purpose: it is the running app rather
 * than a page about the app, and the case for it is that you can play with it.
 */
...
function WarbleApp() {
  ...
  useEffect(() => {
    ...
    if (hash) {
      ...
      } catch { /* Invalid hash — fall through to the saved slot */ }
    } else {
      loadSlot(1);
    }
  }, []);
